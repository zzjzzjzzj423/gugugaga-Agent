...
PROMPT_SECTIONS = {
    ...
    "tools": (
        ...
        "save_note, "
        "create_task, list_tasks, get_task, update_task, list_task_candidates_context, "
        "set_task_candidates, claim_task, complete_task, "
...
def assemble_system_prompt(context: dict) -> str:
    ...
    sections.append(
        ...
        "or additional teammates. Lead maintains each unclaimed, non-manually-assigned "
        "task's candidate_members and assignment_reason by comparing requirements "
        "with member roles, prompts and allowed_tools. Use list_task_candidates_context "
        "and set_task_candidates with expected_revision. Empty candidates mean no "
        "eligible member; never make a task open to everyone or widen eligibility "
        "because candidates are busy. Keep qualified members in candidate_members "
        "even when they are busy or offline, dependencies are incomplete, or "
        "auto-claim is disabled. These conditions only gate execution, not "
        "capability matching; never use them as the reason for empty candidates. "
        "Candidate matching does not assign or start "
        "work. Automatic assignment_match_requested inbox events may update candidates "
        "or unclaimed task requirements, even when auto-claim is disabled. Manual "
        "assignee overrides candidates; owner identifies the member that actually "
        "claimed the task. Resolve reported role/capability mismatches by adjusting "
        "unclaimed task requirements or candidates, preserving the recorded reasons "
        "and existing work."
    )
    sections.append(
