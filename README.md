...
左侧“记忆测评”提供历史结果、新建实验、后台进度和逐题诊断。修改代码后重启 Web Console 即可使用，无需构建前端。

1. 选择基础评测、组件对照或稳定性模板，也可自行增加方案并修改检索、摘要生成、Gate、路由、配额、Top K、预算和模型。
2. 选择单会话 20 题、原实验冻结的 200 题或自选会话/题目；自选的题目上限按每个会话计算。点击“检查配置与差异”，确认题数与各组改动后启动。
3. 页面按后台真实事件显示准备、回放、记忆整合、索引、检索、回答和评分，列出当前方案、会话、题目、重复轮次和实际完成量。点击阶段可展开日志，逐题页可查看候选检索、融合、重排、配额及最终注入内容。
4. 可以停止并从检查点继续。已发出的模型请求会先结束；刷新页面不影响后台运行，服务重启后需手动继续。数据、配置或执行代码改变时需新建实验，避免混合不同协议的结果。

新实验的配置、冻结题目、事件、结果、检查点和数据库均保存在 `<workspace>/.gugugaga/evaluations/<run_id>/`，与日常用户记忆分开。复用历史记忆时先创建一致性快照，每组再使用自己的副本；整合、重建索引和召回反馈只修改实验库。实验执行单独排队，不会作为聊天任务运行。API Key 只在内存中用于模型调用，不写入实验产物；正式启动会使用已配置服务并产生相应调用费用。

成绩页展示 Token F1、BLEU-1、分类/会话成绩、来源关联命中与完整原文命中。F1 是文本重合指标，缺失结果不计为零；缺少完整产物的历史记录会明确标识。稳定性可选择每轮重建全部记忆，或固定注入上下文、只重复生成答案。当前数据源为仓库内 LoCoMo-Refined，尚不支持外部数据集上传和自动网格搜索。

实施任务清单见 [记忆测评实施计划](docs/plans/memory-evaluation-implementation.md)。

#### 构建和复用记忆快照

在“记忆测评”右上角点击“记忆快照”，选择“构建新快照”。可以构建冻结 200 题对应的 10 段完整历史、单会话或自选会话；只需配置记忆生成、整合模型及可选 Embedding 模型。预览会显示会话数、Exchange 数和预计摘要批次；预览不会调用模型，点击“开始构建快照”后才排队执行。

构建任务只回放、整合和建立索引，不生成答案，也没有 Token F1。页面同时显示全任务已完成会话数与当前会话进度，支持停止和从检查点继续。摘要输出格式或长度校验失败最多额外重试 3 次，分别等待 5、15、30 秒，等待时可以停止；网络错误沿用 Provider 已有重试，不再叠加额外网络重试。

完成后，快照管理显示名称、覆盖会话、模型、事实/事件/原文数量及构建参数。点击“用此快照创建 A/B 实验”会创建草稿，两组固定相同来源，默认比较是否展开 Exchange；复用不会再次整合摘要，检索方式和回答模型仍可以各自配置。选择快照时必须覆盖全部待测会话。

已有本地实验处于已完成、已停止、已中断或失败状态时，可在详情点击“保存已完成记忆为快照”，选择方案与重复轮次并命名。仅导出构建完整的会话；部分覆盖明确显示，不会把未完成的记忆当成完整快照。导出使用只读一致性备份，不调用模型。源快照发布到 `<workspace>/.gugugaga/evaluations/snapshots/`，后续实验各用独立副本；清单记录来源、构建配置、代码协议和数据库内容哈希。

升级此功能后需要重启 Web Console。原有实验结果和数据库仍保留；旧实验受代码协议校验限制，不能在新执行代码下直接续跑，但其中已构建完整的会话可以导出并用于新实验。历史快照缺失的构建信息显示“未记录”，不推断成当前默认配置。需求和验收说明见 [记忆快照构建与复用 PRD](PRD-记忆快照构建与复用.md)。
...
pending 任务的管理变更与领取共用该 Task 的短锁。依赖结构变更先获取 `.tasks/.locks/dependencies.lock`，再按固定顺序获取相关 Task 的锁；检查依赖图和保存修改在同一临界区内完成，有环则拒绝保存并返回闭合路径。创建任务时，在新 Task 对应的锁内检查 ID 冲突并原子发布，冲突则换 ID 重试；无依赖的新建任务保留并发路径。Queue 为指定 Assignee 创建 pending Task；同一 Assignee 的 FIFO 队列创建使用 `.tasks/.locks/queue-<sha256>.lock` 协调顺序，不同成员的队列可并发创建。

已有的依赖环会在调度前被识别：环内待执行任务及等待该环的任务不能领取，Lead 收到包含环路的 Mailbox 通知，看板显示“依赖成环”。当前进程内，同一工作区中未变化的环只通知一次；Lead 修正 `blockedBy` 后，任务重新按正常依赖和候选规则调度。暂停根据依赖关系实时计算，任务仍保持 `pending`。运行时已完成的依赖视为已满足。领取任务时只在读取依赖快照期间短暂持有图锁，等待任务锁和写入 Owner 时不持有图锁。
...
候选名单只表示能力是否符合要求。成员忙碌或离线、任务依赖未完成、自动领取关闭，都不能作为清空候选的理由；这些条件由领取阶段单独检查。最后一个依赖完成时会重新评估空候选或尚未完成的匹配，避免之前按阻塞状态给出的空结果一直保留。
...
| 最后一个依赖完成 | 对本次解除阻塞、未领取且未人工指定的任务，若候选为空或匹配仍在进行，递增版本并重新匹配；保留已经确认的非空候选 |
...
依赖成环时优先显示环路及等待 Lead 修正的原因。其余人工指定任务优先检查依赖和指定成员状态；其他任务按不匹配报告、匹配状态、依赖、自动领取开关、候选可用性的顺序给出主要等待原因。自动领取条件全部通过时，`waiting_reason` 为 `null`。
...
- `hermes`：保留会话开头和近期尾部，将待摘要历史及已有摘要一次性提交给摘要模型，不按字节分块。请求超限或摘要校验失败时不提交新的压缩结果。
- `pi`：根据 Token 预算寻找工具协议安全切点，将待移除历史及已有摘要完整提交给摘要模型，不按字节分块；切点位于轮次内部时，另行完整摘要该轮次被移除的前缀。摘要成功且满足预算后生成 Compaction Entry。
